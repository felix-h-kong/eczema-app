...
if [ "$1" = "--prod" ]; then
  # Prod mode: build frontend, serve everything via uvicorn, sync to Android
  cd frontend && npm run build && npx cap sync android && cd ..
  ...
  # Capacitor app loads from the Vite dev server for hot reload (configured in capacitor.config.ts)
